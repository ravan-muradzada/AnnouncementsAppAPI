...
{
    public class EnableTwoFactorAuthTest : AuthServiceTestBase
    {
        #region Success
        [Fact]
        public async Task EnableTwoFactorAuth_Success_DoesNotThrowException()
        {
        ...
        #region UserNotFound
        [Fact]
        public async Task EnableTwoFactorAuth_UserNotFound_ThrowsObjectNotFoundException()
        {
            Guid userId = Guid.NewGuid();
            _userManagerMock.Setup(um => um.FindByIdAsync(It.IsAny<string>()));
